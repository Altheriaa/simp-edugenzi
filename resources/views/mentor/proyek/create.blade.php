...
<div class="max-w-2xl space-y-6">
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-0.5">Isi detail proyek yang akan Anda kelola</p>
        </div>
    </div>

    <div class="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
        <form action="{{ route('mentor.proyek.store') }}" method="POST">
            @csrf
            <div class="space-y-5">
                {{-- Nama Proyek --}}
                <div>
                    ...
                    <input type="text" id="nama_proyek" name="nama_proyek" value="{{ old('nama_proyek') }}"
                ...
                {{-- Deskripsi --}}
                <div>
                    <label for="deskripsi" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                        Deskripsi
                    </label>
                ...
                {{-- Status --}}
                <div>
                    <label for="status_proyek" class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
                    ...
                    <select id="status_proyek" name="status_proyek"
                        ...
                        <option value="berjalan" @selected(old('status_proyek', 'berjalan') === 'berjalan')>Berjalan</option>
                        <option value="tertunda" @selected(old('status_proyek') === 'tertunda')>Tertunda</option>
                        <option value="selesai" @selected(old('status_proyek') === 'selesai')>Selesai</option>
                    </select>
                </div>
            </div>

            <div class="mt-6 flex items-center justify-end gap-3">
                <a href="{{ route('mentor.proyek.index') }}"
