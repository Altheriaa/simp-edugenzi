...
<div class="space-y-6">
    <div>
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Riwayat penilaian bintang EAC dari mentor per program pelatihan yang Anda ikuti.</p>
    </div>
    ...
    @if ($enrollments->isEmpty())
        <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 py-16 text-center text-gray-400">
            ...
            <p class="text-sm">Anda belum terdaftar di program pelatihan apa pun.</p>
        </div>
    @else
        <div class="space-y-8">
            @foreach ($enrollments as $enrollment)
                <div class="space-y-4">
                    <h2 class="text-lg font-bold text-gray-800 dark:text-gray-200 border-b pb-2 dark:border-gray-700">
                        {{ $enrollment->programPelatihan->nama_program ?? 'Program' }} 
                        <span class="text-sm font-normal text-gray-500">({{ $enrollment->status }})</span>
                    </h2>

                    @if ($enrollment->penilaian->isEmpty())
                        <div class="rounded-xl border border-dashed border-gray-300 bg-gray-50 dark:border-gray-700 dark:bg-gray-800/50 py-8 text-center text-gray-500">
                            Belum ada penilaian untuk program ini.
                        </div>
                    @else
                        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3">
                            @foreach ($enrollment->penilaian as $penilaian)
                                <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900 overflow-hidden">
                                    ...
                                    <div class="bg-gradient-to-r from-blue-500 to-indigo-600 px-5 py-4 flex items-center justify-between">
                                        <div>
                                            ...
                                            <p class="text-white/70 text-xs">oleh {{ $penilaian->mentor->nama_lengkap ?? 'Mentor' }}</p>
                                        </div>
